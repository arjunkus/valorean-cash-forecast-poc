...
DEFAULT_ARCHIVE_PATH = 't1_position_history.parquet'


def archive_position(manager, storage_path=DEFAULT_ARCHIVE_PATH):
    """Archive a built position with full category-level detail."""
    if manager.position_df is None:
        manager.build_position()
    df = manager.position_df
    summary = manager.get_position_summary()

    record = {
        'position_date': manager.position_date,
        'opening_balance': manager.opening_balance_actual,
        'closing_forecast': summary['closing_forecast'],
        'closing_actual': summary['closing_estimated'],
        'absolute_error': abs(summary['closing_estimated'] - summary['closing_forecast']),
        'bias': summary['closing_estimated'] - summary['closing_forecast'],
        'bias_direction': 'Over' if summary['closing_estimated'] > summary['closing_forecast'] else 'Under',
        'posted_transactions': summary['posted_transactions'],
        'scheduled_payments': summary['scheduled_payments'],
        'archived_at': datetime.now(),
    }

    # Flatten category-level detail
    for _, row in df.iterrows():
        cat = row['category']
        if cat in ('OPENING_BALANCE', 'CLOSING_BALANCE', 'NET_CASH_FLOW'):
            continue
        if cat in ('TOTAL_INFLOW', 'TOTAL_OUTFLOW'):
            prefix = 'total_inflow' if cat == 'TOTAL_INFLOW' else 'total_outflow'
            record[f'{prefix}_forecast'] = row['forecast']
            record[f'{prefix}_actual'] = row['estimated_actual']
        else:
            record[f'{cat}_forecast'] = row['forecast']
            record[f'{cat}_actual'] = row['estimated_actual']
            record[f'{cat}_variance'] = row['variance']

    record['net_cash_flow_forecast'] = record.get('total_inflow_forecast', 0) - record.get('total_outflow_forecast', 0)
    record['net_cash_flow_actual'] = record.get('total_inflow_actual', 0) - record.get('total_outflow_actual', 0)

    new_row = pd.DataFrame([record])

    if os.path.exists(storage_path):
        existing = pd.read_parquet(storage_path)
        # Remove duplicate for same position_date
        existing = existing[existing['position_date'].dt.date != manager.position_date.date()]
        combined = pd.concat([existing, new_row], ignore_index=True)
    else:
        combined = new_row

    combined.to_parquet(storage_path, index=False)
    return combined


def get_historical_accuracy(storage_path=DEFAULT_ARCHIVE_PATH):
    """Load historical archive and return the DataFrame."""
    if not os.path.exists(storage_path):
        return pd.DataFrame()
    return pd.read_parquet(storage_path)


def seed_historical_archive(forecaster, num_days=30, storage_path=DEFAULT_ARCHIVE_PATH):
    """Generate num_days business days of realistic historical archive data."""
    ...
    opening_balance = forecaster.last_actual_closing_balance

    # Walk backwards to find business days
    dates = []
    d = position_date
    ...
    records = []
    running_balance = opening_balance * 0.95  # start slightly different

    for day in dates:
        mgr = DailyPositionManager()
        mgr.initialize_position(
            position_date=day,
            opening_balance_actual=running_balance,
            forecast_df=t1_forecast,
        )
        intraday = simulate_intraday_data(day, t1_forecast)
        mgr.load_intraday_from_bank(intraday['bank_transactions'])
        mgr.load_sap_payment_queue(intraday['sap_payments'])
        mgr.build_position()
        ...
        # Evolve running balance with slight random walk
        np.random.seed(day.toordinal())
        running_balance = summary['closing_estimated'] + np.random.uniform(-200000, 200000)

    archive_df = pd.DataFrame(records)

    # Merge with any existing data (keep existing rows for dates not in seed)
    if os.path.exists(storage_path):
        existing = pd.read_parquet(storage_path)
        seed_dates = set(archive_df['position_date'].dt.date)
        existing = existing[~existing['position_date'].dt.date.isin(seed_dates)]
        archive_df = pd.concat([existing, archive_df], ignore_index=True)

    archive_df = archive_df.sort_values('position_date').reset_index(drop=True)
    archive_df.to_parquet(storage_path, index=False)
    return archive_df


def clear_archive(storage_path=DEFAULT_ARCHIVE_PATH):
