...
def render_overview():
    ...
    with col1: st.metric("T0 Balance", format_currency(forecaster.last_actual_closing_balance))
...
def render_daily_position():
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("Opening", format_currency(summary['opening_balance']))
    with col2: st.metric("Closing (Forecast)", format_currency(summary['closing_forecast']))
    ...
    with col2: st.metric("Target Balance", format_currency(rec['target_balance']))
...
def render_forecasts():
    ...
    with col1: st.metric("Opening", format_currency(forecaster.last_actual_closing_balance))
    with col2: st.metric("+ Receipts", format_currency(fcast['forecast_inflow'].sum()))
    with col3: st.metric("- Payments", format_currency(fcast['forecast_outflow_ex_capex'].sum()))
    with col4: st.metric("- CAPEX", format_currency(fcast['CAPEX'].sum()))
    with col5: st.metric("= Closing", format_currency(fcast['closing_balance'].iloc[-1]))
            ...
            if len(sev_df) > 0:
                ...
                for _, row in sev_df.iterrows():
                    with st.expander(f"{row['date'].strftime('%Y-%m-%d')} - {row['anomaly_type']}", expanded=(sev=='High')):
                        ...
                        with c1: st.metric("Actual", format_currency(row['value']))
                        with c2: st.metric("Expected", format_currency(row['expected']))
...
def format_xaxis_by_horizon(fig, horizon, dates):
    """Format X-axis labels appropriate for the time horizon."""
    if horizon == 'T+1':
        ...
        # Weekly - show dates at interval
        fig.update_xaxes(
            tickformat='%b %d',
            dtick=86400000,  # Show every day but can be adjusted
            title_text='Date (7-Day Forecast)'
        )
    elif horizon == 'T+30':
        # Monthly - show week intervals
        fig.update_xaxes(
            tickformat='%b %d',
            dtick=86400000 * 7,  # Weekly intervals
            title_text='Date (30-Day Forecast)'
        )
    elif horizon == 'T+90':
        # Quarterly - show monthly intervals
        fig.update_xaxes(
            tickformat='%b %Y',
            dtick='M1',  # Monthly
            title_text='Date (90-Day Forecast)'
        )
    return fig


def format_currency(value):
    """Format large currency values with M/K suffix."""
    abs_val = abs(value)
